...
class gamma_simulator:
    def __init__(self,
        ...
        self.lambda_n = round(self.lambda_value * self.dt,2)  # = lambda_value / fs
        # --- load the spectrum ---
        # hist_energy: energy values [keV] (bins, x-axis
        # hist_counts: amount of counts for each hist_energy value (y-axis)
        ...
        if 'name' in source.keys() and 'weights' in source.keys():
            ...
            self.hist_energy, self.hist_counts, self.train_hist_energy = self.load_weighted_spectrum(source)
    ...
    def verbose_plot_energy(self):
        ...
        plt.bar(self.hist_energy, self.hist_counts, width=2, alpha=0.5)
        plt.yscale('log')
        ...
        if type(self.energy_desc)==list:
            #convert list to str
            energy_desc = f'{self.energy_desc}'
        else:
            energy_desc = self.energy_desc
        
        plt.savefig('/root/WorkSpace/project/spectrum_two_stage/results/figs/label_'+
                    f'{round(self.lambda_n,4)}'+'_'+energy_desc+'_bins_'+f'{self.bins}'+'.png', format='png')
        plt.close()
    ...
    def load_spectrum_data(self, source) -> 'tuple[np.ndarray, np.ndarray]': # new lines: add "spectra" to local
        ...
        with open('/root/WorkSpace/project/spectrum_two_stage/database/spectra/'+ source +'.html') as html_file:
        ...
        # new lines: choose bins
        if self.bins == 512:
            # convert 8192 bins to 512 bins
            new_x = np.zeros(512).astype(np.float64)
            new_y = np.zeros(512).astype(np.float64)
            train_hist_energy = np.arange(512)+1
            for i in range(512):
                new_x[i] = np.sum(x[16*i:16*i+16])/16
                new_y[i] = np.sum(y[16*i:16*i+16])
            energy, counts = new_x, self.normalize_histogram(new_y)
        if self.bins ==1024 :
            # convert 8192 bins to 1024 bins
            new_x = np.zeros(1024).astype(np.float64)
            new_y = np.zeros(1024).astype(np.float64)
            train_hist_energy = np.arange(1024)+1
            for i in range(1024):
                new_x[i] = np.sum(x[8*i:8*i+8])/8
                new_y[i] = np.sum(y[8*i:8*i+8])
            energy, counts = new_x, self.normalize_histogram(new_y)
        elif self.bins == 2048:
            # convert 8192 bins to 2048 bins
            new_x = np.zeros(2048).astype(np.float64)
            new_y = np.zeros(2048).astype(np.float64)
            train_hist_energy = np.arange(2048)+1
            for i in range(2048):
                new_x[i] = np.sum(x[4*i:4*i+4])/4
                new_y[i] = np.sum(y[4*i:4*i+4])
            energy, counts = new_x, self.normalize_histogram(new_y)
        elif self.bins == 4096:
            # convert 8192 bins to 4096 bins
            new_x = np.zeros(4096).astype(np.float64)
            new_y = np.zeros(4096).astype(np.float64)
            train_hist_energy = np.arange(4096)+1
            for i in range(4096):
                new_x[i] = np.sum(x[2*i:2*i+2])/2
                new_y[i] = np.sum(y[2*i:2*i+2])
            energy, counts = new_x, self.normalize_histogram(new_y)
        elif self.bins == 8192:
            energy, counts = x, self.normalize_histogram(y)
            train_hist_energy = np.arange(8192)+1
        
        return energy, counts,train_hist_energy
    ...
    def load_weighted_spectrum(self, source) -> 'tuple[np.ndarray, np.ndarray]':
        ...
        for i, source_name in enumerate(source['name']):
            hist_energy, hist_counts, train_hist_energy = self.load_spectrum_data(source_name)
        ...
        return total_hist_energy, total_hist_counts, train_hist_energy
    ...
    def generate_energy_distribution(self) -> np.ndarray:
        ...
        np.random.seed(self.seed)
        energies = np.random.choice(self.train_hist_energy,
                                    size=self.n_events,
                                    replace=True,
                                    p=self.hist_counts)
        return energies
    ...
    def evaluate_shape_len(self) -> 'tuple[int, float, float]':
        ...
        if self.dict_type == 'double_exponential':
            ...
            if self.dict_shape_params['custom']:
                ...
                shape_time = stats.gamma.ppf(0.995, self.dict_shape_params['param1bins'].max() - 1, # should not minus 1
    ...
    def generate_signal(self) -> np.ndarray:
        ...
        if self.verbose_plots['signal']:
            ...
            plt.axis('tight')
            plt.grid()
            plt.savefig('/root/WorkSpace/project/spectrum_two_stage/results/figs/gamma_simulation.png', format='png')
            plt.close()
        if self.verbose:
            self.verbose_info()
        # return signal
